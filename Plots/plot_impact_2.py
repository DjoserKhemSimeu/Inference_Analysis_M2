...
df_gwp_water = xls.parse('Water', header=3)
df_gwp_gwp = xls.parse('GWP', header=3)


all_dates = pd.concat([data['date'], data['date']])
norm = mcolors.Normalize(vmin=all_dates.min(), vmax=all_dates.max())


def generate_scatter(ax, df_gwp, title, ylabel):
        ...
        if category in ["Literature", "Database"]:
            ...
            if not pd.isna(year_value):
                ...
                if category == "Industry":
                    indices = [idx for idx in indices if data.loc[idx, "foundry"] == author]
                for idx in indices:
                    hardware_dict[names[idx]].append(value)

    hardware_means, hardware_tdp, hardware_names, hardware_errors, hardware_dates = [], [], [], [], []
    ...
    scatter = sns.scatterplot(x="TDP", y="Mean Impact Environnemental", data=df_scatter, ax=ax,
                              hue="Date", palette='viridis', legend=None, s=100, norm=norm)
...
generate_scatter(axs[0], df_gwp_water, 'Manufacturing impact - TDP (Water Consumption)', 'Water Consumption (L)')
generate_scatter(axs[1], df_gwp_gwp, 'Manufacturing impact - TDP (GWP)', 'Global Warming Potential (kgCO2 eq)')
