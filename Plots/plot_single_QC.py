...
impact_conception = {
    ...
    'NVIDIA-Quadro-RTX-8000': [24.965243,43.519200 ],
    'NVIDIA-Orin': [25.539700, 35.895500]# Exemple de valeurs pour le Quadro RTX 8000, à remplacer par les vraies données
}
...
for qc in qc_values:
    ...
    data_2080Ti = pd.read_csv(f'data/consommation_energie_2080Tix1_BS_1_QC_{qc}.csv')
    ...
    data_orin = pd.read_csv(f'data/consommation_energie_single_orin_QC_{qc}.csv')
    ...
    timestamp_orin = data_orin['timestamp']  # sec
    gpu_power_orin = data_orin['gpu_power'] / 1000  # mW -> W
    area_orin = np.trapz(gpu_power_orin, timestamp_orin)
    energie_totale_orin_kWh = area_orin / 3_600_000
    Co2_emissions_orin = energie_totale_orin_kWh * EGM
    impact_total_orin_values = [Co2_emissions_orin + ((timestamp_orin.iloc[-1] / seven_years) * value * 1000) for value in impact_conception['NVIDIA-Orin']]
    ...
    results.append({
        'QC': qc,
        'GPU': 'NVIDIA-Orin',
        'Impact Environnemental Total (g)': np.mean(impact_total_orin_values),
        'category': 'Edge'
    })
