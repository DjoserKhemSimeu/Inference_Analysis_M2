...
+ Hidden size = size of query, key, value and embedings vectors
+ Intermediate size = number of hidden layer in the FFN
...
+ "In fact, for Facebook, hardware production represent 30% of the total carbon footprint of ”large scale ML tasks”"
+ "The objective of MLCA is to allow ML practitioners to estimate the environmental impacts/ benefits balance of their experiments and decide if it is worth pursuing them."
*** Observations :
+ Machine Learning life Cycle Assessment (MLCA) : aimed at providing researchers with LCA estimates of computation impact that can be obtained independently of running calculations.
+ The carbon footprint is computed by all tools as follows:
#+begin_example
GWP = CI ∗ PUE ∗ (p_c + p_g + p_m) ∗ t
where :
p_c = power consumption of CPU
p_g = power consumption of GPU
p_m = power consumption of memory
PUE = Power usage efficiency
CI = Carbon Itensity of the electicity powering computation (grid mix)
GWP = Global Warming Potential
#+end_example
+ Attributional LCA : ”What are the impacts of transporting 10,000 people a day by bus over 15 km?”
+ Consequencial LCA : ”Given the current bus network, what would be the impacts of adding 1,000 new passengers a day?”
+ MLCA : ”What are the impacts of running program X on the hardware Y during Z hours?”
+ "An analysis of different phases of the life cycle of hardware Y is required to evaluate the different impacts each considered part of the life cycle has."
#+begin_center
[[file:images/MLCA_Table.png]]
[[file:images/MLCA_gpu_impact.png]]
#+end_center
+ docs : https://github.com/blubrom/MLCA/blob/main/docs/docs/Explanations/manufacture-components.md
** Tuesday 11 february :
*** MLCA Tool :
I failed to use the tool via the GitHub repository. But maybe it enough to compare the die size en memory size to observe that the edge's device have the potential to have less impact compare to the large scale
**** Data :
***** NVIDIA Jetson AGX Xavier 32 GB (Edge device):
+ Die size = 350 mm²
+ Transistors = 9000M
+ TDP = 30W
+ L1 cache = 128KB/SM
...
+ Tensor cores = 64
+ Architecture = Volta
***** NVIDIA A100 SXM4 - 40G (Harware used):
+ Die size = 826 mm²
+ Transistors = 54200M
+ TDP = 400W
+ L1 cache = 192KB/SM
...
+ Tensor cores = 432
+ Architecture = Ampere
*** Recall on the PBQ : Which conditions allow edge computing deployments to be more efficient than large-scale deployments for inference?
  + Efficiency : In our context we can consider efficiency with a QoE approach where the cost refer to the environemental impact + latencty.
**** Hypothesis :
+ by knowing that the die area of the Esge device that we want to study is less important than the ones used for large scale deployment we can expect that the manufacturing impact of edge devices have a less important impact.
[[file:images/Inf_Formula.png]]
+ With this impact formula for the inference process, we can that :
  + The imapct of the hardware is mesured by Cie and Fe
  + Fe : The impact of the GPU used is ponderated by the allocation time of the hardware
  + Cie : The Electicty comsuption of running i with the hardware e depend on the time of usage of e with i
+ Observations : for the same allocation time of the device e (same percentage of use) the inference impact of edge devices is less important than large scale):
  + Cie = TDP * t
  + Fe = Die size
***** Problem : With the same model ae(t) will be much higher for edge device : Starcoder case :
+ StarCoder size : 15,5B
  + float32 ~ 15.5 * 10⁹ * 4 bytes ~ 57.74 GB (incompatibe with NVIDIA jetson hardware)
+ StarCoder complexity : l= size of the sequence; d = embeddsing size; f = size of the intermediate layer of the FFN
  + By considering only the multi query attention mechanism and the feed forward network for the layer of the decoder only architecture: (source : Attention is all you need)
    + Mutli-query attention >~ O(l²*d) = 8192² * 6144 ~ 4.12e11
    + FFN >~ O(l*d*f) = 8192 * 6144 * 24576 ~ 1.24e12
  + Total ~ 1.65e12 Flop for 1 decoder bloc → 40*1.65e12 ~ 6.6e13 = Lower bound of the number of FLOPs per forward pass
****** Roofline model :
+ Nvidia A100 SXM4 - 40G :
  + Bandwidth : 1.56T/s
  + Performance : 19.49 TFLOPS
+ Nvidia Jetson AMX Xavier 32 GB :
  + Bandwidth : 136.5GB/s
  + Performance : 1.410 GFLOPS
Computational intensity of 1 inference : 6.6e4/57.74 ~ 1143.0.5 FLOPS/Byte
[[file:images/Roofline_model.png]]
+ We can see that in both settings, the inference computation will be limited by the peak performance of the GPU used
+ The limitation is really hard for Jetson (~ perf divided by 10 000)
+ Time : FLOPs/FLOPS
  + Jetson ~ 46808.51 seconds ~ 13 hours
  + A100 ~ 3.38 seconds
+ To mesure the inference cost of both settings by following the inference formula we can set :
  + PUE = 1
  + EGM = 1
  + A100 :
    + Cie = 400 * 3.38 = 1352
    ...
    + I ~ 2178
  + AGX Xavier :
    + Cie = 30* 46808.51 = 1404255.3
    + Fe ~ 350
    + I ~ 1404605.3

** 10° paper : Life-Cycle Emissions of AI Hardware: A Cradle-To-Grave Approach and Generational Trends
*** Authors :
Ian Schneider, Hui Xu, Stephan Benecke, David Patterson, Keguo Huang, Parthasarathy Ranganathan, and Cooper Elsworth (Google)
*** Keywords :
LCA, AI accelerator, Tensor Processing Units (TPUs)
*** SOTA :
*** Observations :
[[file:images/TPU_ctg.png]]
+ Google’s AI-focused application-specific integrated circuits (ASICs) : offer substantial improvements in training and inference speeds for large-scale machine learning models.
[[file:images/TPU_Results.png]]
+ compute carbon intensity (CCI) : the CO2e per utilized floating-point operation (FLOP), or CO2e/FLOP
[[file:images/TPU_Results_2.png]]
+ "Trillium (TPU v6e), launched to customers in December 2024, offers a 3x improvement on CCI versus v4i."
  + This study don't take into account the ADP
  + They not devolloped their methods to measure the manufacturing impact
  + The normalzition by performance also smooth the CO² estimation
  + The substarction of "clean eneregy" seems a bit strange (0 impacts? Really ?)
** 11° paper : SmoothQuant: Accurate and Efficient Post-Training Quantization for Large Language Models (https://openreview.net/pdf?id=sHfSV8eYEp)
*** Authors :
Guangxuan Xiao, Ji Lin, Mickael Seznec, Hao Wu, Julien Demouth, Song Han
*** Date : 2023
*** Keywords :
Large language models (LLMs), post-training quantization (PTQ), accuracy-preserving
*** Citations :
+ "By quantizing the weights and activations with low-bit integers, we can reduce GPU memory requirements, in size and bandwidth, and accelerate compute-intensive operations."
...
+ "Previous work has shown that quantizing the weights of LLMs with INT8 or even with INT4 does not degrade accuracy"(uniform/flat distribution)
*** Observations :
 [[file:images/SmoothQuant_intro.png]]
**** Quantization :
+ maps a high-precision value into discrete levels.
  [[file:images/Quant_formula.png]]
+ Static quantization : "We can calculate ∆ offline with the activations of some calibration samples"
+ Dynamic quantization : "We can also use the runtime statistics of activations to get ∆"
+ "LLMs are notoriously difficult to quantize due to the outliers in the activations" + "The scale of outliers in activations is ∼ 100× larger than most of the activation values."
  [[file:images/Quant_window.png]]
  [[file:images/Quant_transformer.png]]
** 2° survey : A Survey on Transformer Compression (https://arxiv.org/pdf/2402.05964)
*** Quantization :
[[file:images/Trans_quant.png]]
*** Knowledge Distillation :
[[file:images/Trans_KD.png]]
**** TODO :: Papers to read :
+ TinyBERT : X. Jiao et al. Tinybert: Distilling bert for natural language understanding. arXiv preprint arXiv:1909.10351, 2019.
*** Efficient architecture :
[[file:images/Trans_arch.png]]
*** TODO :: For next time :
+ Deeply define the part of the mind map on PBQ, Case Study, and the Estimation to define what we can do from these estimations
** Article : AI energy score (https://huggingface.github.io/AIEnergyScore/):
[[file:images/HF_mem.png]]
[[file:images/HF_EGM.png]]
+ ADEME : https://base-empreinte.ademe.fr/
+ "However, results on alternative hardware should be interpreted cautiously, as thez will not align  directly with the GPU-specific metrics established in this study."
...
** Grid5000 : GPU distribution among sites :
[[file:images/G5k_GPU.png]]
[[file:images/G5k_edge.png]]
+ Manufacturing impact :
  #+begin_example
  (norm(Die_area/transistro_size)+norm(MemGB))/2
  #+end_example
+ Scale level :
  #+begin_example
  (norm(manufacturing_impact)+norm(TDP))/2
  #+end_example
+ Normalization :
  #+begin_example
  norm(x)=(x-min(x))/(max(x)-min(x))
  #+end_example
+ Grid'5000: Adding support for experimentation from core-network to edge devices : https://www.grid5000.fr/Core2EdgeNetworkXP/
+ price source : https://smicro.eu/search?search=nvidia%20l40
** Experimental methodology
*** Application :
StarCoder inference.
*** Hadware to use :
Goal : compare environnemental impact of different hardware depending of their edge level:
+ Select a sample of hardware available and observe the environnemental impact of the inference process :
  + How to measure energy consumption of the hardware during the inference process :
    + CodeCarbon(Nvidia-smi): allow us to follow the eneregy consumption of the device during inference.
      "Specifically, CodeCarbon leverages the NVIDIA System Management Interface (nvidia-smi), a command-line utility, to measure GPU energy consumption. Furthermore, it provides a detailed breakdown of energy contributions from individual steps such as preprocess, prefill, and decode."(https://mlco2.github.io/codecarbon/)
  + How to meausure the PUE ?
+ How many inference do we consider ?
  + Same as the Copilot study ?
  + Just one inference process ?
+ How do we handle memory requirement for the StarCoder model ?
  + By considering only GPU with enough memory ? (don't consider some edge devices)
  + Quantization ?
    + Only for GPU with not enough memory? (different models ~> different performances)
    + For all GPU ?
*** Text Generation Inference (TGI) : https://www.adyen.com/knowledge-hub/llm-inference-at-scale-with-tgi
+ toolkit for deploying and serving Large Language Models (LLMs)
+ API Documentation : https://huggingface.github.io/text-generation-inference
[[file:images/TGI_arch.png]]
+ Prefill : tokenization of the prompt on CPU -> GPU
+ Decode : autoregressive nature of the LLM (genrate output one token at time)-> De-tokenized on CPU
+ "The separation of the Prefill and Decode stages is essential due to the distinct computational characteristics of each stage."
+ "The engine handles everything related to the models and batching requests, while the server focuses on forwarding user requests. In TGI, these components are named accordingly: the server is referred to as the router"
[[file:images/TGI_arch_2.png]]
** Review meeting friday 21 february :
*** Enviornmental impacts :
+ The metrics choosed by the StableDiffusion study was choosed by the author, we can consider others metrics.
+ For the PUE we can use some approximation/estimation of datacenters.
+ Fe= manufacturing impact.
+ The term deployment is not the right one.
*** StarCoder case study :
+ maybe not enough performant we must check for other models
*** Experimental methodology :
+ Work with danilo about the manufacturing impact and the edge level
+ Change the term edge level
+ Change the term deployment
+ No Code Carbon -> Use Kwollect
** 12° paper : The Environmental Footprint of IC Production: Review, Analysis, and Lessons From Historical Trends (https://ieeexplore.ieee.org/abstract/document/9979766)
*** Keywords :
environmental impacts of integrated circuit (IC), energy consumption, carbon footprint, water consumption
*** SOTA :
*** Citations :
+ ". A significant part of this footprint, i.e., between 360 and 660 MtCO2 eq [2], is attributed to the production phase. More specifically, the production of ICs is known to be responsible for an important share of cradle-to-gate impacts [3], [4], [5], [6], [7], [8], [9], therefore justifying special attention."
+ "In this study, we normalize all environmental indicators by the silicon die area in cm2"
*** Observations :
+ Foundries have to annually report their environmental performance in corporate sustainability responsibility (CSR) reports which ensure a systematic source of information.
...
+ Scientific literature is an important source of data as several peer-reviewed studies have been carried out over the last 20 years.
+ Databases from LCA tools provide specific data and usually benefit from regular updates.
[[file:images/IC_node.png]]
[[file:images/IC_Found.png]]
** Observation of the difference in the impact of the memory type
+ DRAM = Dynamic random access memory
*** High Bandwith Memory (HBM):
+ SDRAM = Synchronous Dynamic RAM
+ higher bandwidth than  DDR or GDDR5 while using less power.
+ Stacking DRAM dies and an optional base die which can include buffer circuitry and test logic.
+ Within the stack the dies are vertically interconnected by through-silicon vias (TSVs) and microbumps.
  [[file:images/HBM_Table.png]]
  [[file:images/HBM_arch.png]]
+ HBM2 : 256 GB/s memory bandwidth per package (2016), 8‑Hi stacks, 8 GB per stack possible.
+ HBM2e : 307 GB/s memory bandwidth per stack, 12‑Hi stacks, up to 24 GB per stack possible.
+ DIE area approximation (DDR4 = https://www.techinsights.com/blog/techinsights-memory-technology-update-iedm18)
  [[file:images/DD_Table.png]]
  + Bit density HBM2e = HBM2 = 0.137 Gb/mm² (Gb = Gigabits != GB)
*** Graphics Double Data Rate Synchronous Dynamic RAM
+ GDDR5 bit density : Based on DDR3 SDRAM Memory : memory density ~ DDR4 = 0.137Gb/mm²
+ GDDR5X bit density : ~ GDDR5
+ GDDR6 bit density : 2*DDR4= 0.274Gb/mm²
  LPDDR4x bit density : ~ LPDDR4 = 0.152Gb/mm²
** Presentation report 28 february
+ add another dimension in the EIxScale graph : number of devices
+ The term scale level is not the right one -> Supercomputer
+ See with thomas ropars for memory footprint
+ The quadranglesatic regression is not relevant
+ The memory formula must be reworked
*** What to do next :
+ Continue to work with grid 5000 to replace TDP by the effective energy consumption of the devices
+ Write a report about what I 've done so far
+ Think about a poster
* Conférence rennes :
+ Lieu : IRISA sur le campus de Beaulieu à Rennes (https://www.irisa.fr/localisez-nous-rennes)
+ Debut : mardi 25 mars à 13h30
  + Arrivée veille :
    [[file:rennes/Aller_Rennes.png]]
  + Arriver jour meme :
    [[file:rennes/Aller_Rennes2.png]]
+ Fin : mercredi 26 mars à 17h :
    [[file:rennes/Retour_Rennes.png]]
+ Hotel : (https://www.booking.com/hotel/fr/cosylocation-le-bois-perrin.fr.html?aid=357029&label=yho748jc-1FCAMoTUIGcmVubmVzSA1YA2hNiAEBmAENuAEZyAEM2AEB6AEB-AECiAIBqAIDuAK_spK-BsACAdICJDJmYmVlMmExLTNhOGQtNDRhNy04ZTA2LWI4ZDNhYzQzZmZjNdgCBeACAQ&sid=d6fe42cb0d1e27215d54689741a8705d&all_sr_blocks=646972501_265598844_1_0_0&checkin=2025-03-24&checkout=2025-03-26&dest_id=-1461464&dest_type=city&dist=0&group_adults=1&group_children=0&hapos=1&highlighted_blocks=646972501_265598844_1_0_0&hpos=1&matching_block_id=646972501_265598844_1_0_0&no_rooms=1&req_adults=1&req_children=0&room1=A&sb_price_type=total&sr_order=popularity&sr_pri_blocks=646972501_265598844_1_0_0__16326&srepoch=1740937769&srpvid=229b7d3ee330045b&type=total&ucfs=1&)
** Benchmarking phase
The goal of this phase is to simulate an inference server on the Grid5000 GPU nodes to observe data over different experimental settings.
*** TODO :: Problem with Jetson AGX Xavier
+ The Jetson AGX Xavier is available on Grid5000 on toulouse with a Nvidia Caramel GPU based on a arm64  architecture.
+ TGI server is not compatible with arm64 architechture
+ We can inspect the possibility of performing an emulation of an x86 architecture on a arm64 architechture (with Qemu for example)
+ but an emulated environnement could introduce some lack of performance we must check if it introduce some waste of performance in the GPU compare to other settings.
+ Three choices :
  + Perform our experimentation with an emulation
  + Perform our experimentations without taking into account AGX Xavier (with T4 as best edge device)
  + Try to find an other experimental setting instead of TGI
*** What to do next :
+ Investigate over the TGI benchmarking process
+ See which model can be used for experimentations
+ Understand how to measure energy consumption
+ Reading Thomas Ropars's paper
+ prendre en compte la durrée de vie :3. 5 .7
+ miltiplier la densite par gCo2
+ Souces memoire : https://www.techdesignforums.com/practice/technique/understanding-ddr-sdram-memory-choices/
+ https://www.techdesignforums.com/practice/technique/choosing-between-ddr4-and-hbm-in-memory-intensive-applications/
** TODO paper to read : MLPerf Inference Benchmark (https://arxiv.org/pdf/1911.02549)
** Nvidia report 2024 :
Data Center
...
* paper expe : MLPerf Inference Benchmark (https://arxiv.org/pdf/1911.02549)
** Keywords :
Machine Learning , ML inference, Benchmarking
** Citations :
+ "The myriad combinations of ML hardware and ML software make assessing MLsystem performance in an architecture-neutral, representative, and reproducible manner challenging."
** Observations :
+  in offline batch processing, such as photo categorization, all the data may be readily available in (network) storage, allowing accelerators to reach and maintain peak performance.
+ Single-stream. The single-stream scenario represents one inference-query stream with a query sample size of 1, reflecting the many client applications where responsiveness is critical. An example is offline voice transcription on Google’s Pixel 4 smartphone. To measure performance, we inject a single query into the inference system; when the query is complete, we record the completion time and inject the next query. The metric is the query stream’s 90th-percentile latency.
+ Multistream. The multistream scenario represents applications with a stream of queries, but each query comprises multiple inferences, reflecting a variety of industrial-automation and remote-sensing tasks. For example, many autonomous vehicles analyze frames from multiple cameras simultaneously.
+ Server. The server scenario represents online applications where query arrival is random and latency is important. Almost every consumer-facing website is a good example, including services such as online translation from Baidu, Google, and Microsoft.
+ Offline. The offline scenario represents batch-processing applications where all data is immediately available and latency is unconstrained. An example is identifying the people and locations in a photo album. For this scenario, we send a single query that includes all sample-data IDs to be processed, and the system is free to process the input data in any order.
+ querie = request to the inference system
+ sample= inference input.
  + Bert dataset= SQUAD v1.1 (https://rajpurkar.github.io/SQuAD-explorer/explore/1.1/dev/)
+ Doc bert with SQUAD: https://arxiv.org/pdf/1908.01767
* Tuesday 22 april
+ I observed in detail of the energetic performance of different deployment setting on the Jetson and the A100 GPUs
+ in the single-stream scenario :
  + For the A100 we can see that the performance is strongly affected by the modification of the batch size: when the batch size is one the instantaneous power observed is arround 175W during the benchmark, when the BS is 4 150W an 8 100W. We can see that a to large BS affect strongly the runtime : BS 8 -> 325s VS BS 1/4 -> 115s
  + For the Jetson there is no difference when I vary the batch size parameters. This observation can be interpreted as the AGX Xavier device is already use at it's peak performance and increasing the number of inference input to process in parallel incur no improvment because we already apply the maximum parallelism with 1 input inference.
+ in the server scenario :
  + For the A100 , The batch size have no effect on thge performance but the number of query by second have a significant effect on the results, we can see that when we increase the QPS from 360 to 720 the GPU is less under used, the runtime decrease and the average intantaneous power increase.
  + For the Jetson, we can see that increasing the batch size reduce sligthly the runtime but not a lot and the maximum batch for the AGX Xavier is 24 due to lack of device memory.
* Faster is Not Always Better: Understanding the Effect of Dynamic Response Delays in HumanChatbot Interaction (https://web.archive.org/web/20200322152504id_/https://aisel.aisnet.org/cgi/viewcontent.cgi?article=1112&context=ecis2018_rp)
+ faster is not always better
+ Control condition : quasi-instantaneous answer : network latency ~ 1s
+ Treatment condition : dynamic delay depending on the answer complexity : network latency + dynamic delay frel
#+begin_center
[[file:images/delay_bot.png]]
#+end_center
#+begin_center
[[file:images/res_bot.png]]
#+end_center
* Retour Laurent:
- Mettre jour données giec "impossible" (sortir le dernier rapport)
- en gras122.5
- slide 4-5 beaucoup de travaux sur l'apprentissage et peu sur l'inference (deplacer slide 9 vers 4-5) comment le edge peut aider
- en ouverture complémentarité en
- plus parler du edge : bordure de réseau besoin de latence, proch ede l'utilisateur, ressource limité
- plusait insister sur le fait que la discret c'est moi
- Dire l'unité de l'Iinf
- slide 1 I proc
- " la vous avez l'impression que c'est facile mais le challenge etait de definir quelles ressources ont almler utiliser"
- Remplacer FU par Cat
- meilleur qualiter slide 15
- mieux expliquer BERT
- v1.1 v4.1 slide 21 et add MLPerf
- Query Count slides 21
- combiner la latence edge et le réseau  (faible latence slide 23)
