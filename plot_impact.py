...
for i, name in enumerate(names):
    if name in hardware_dict:
        ...
        mem_name = mem_type[i]
        mem_size = mem[i]
        impact_values = [(val * die_area_value*0.1)+((((mem_size*8)/mem_data[mem_name])*0.1)*val) for val in hardware_dict[name]]
...
for i, name in enumerate(df_scatter['Hardware'].unique()):
    ...
    texts.append(ax.annotate(f"{name}", (tdp_value, mean_value), fontsize=10, ha='center',
...
ax.plot(x_range, y_range, color='red', linestyle='--', label="Qudratic regression")


ax.set_xlabel('TDP')
ax.set_ylabel('Manufacturing impact (kgCO2 eq)')
ax.set_title('Manufacturing impact - TDP with confidence intervals')
