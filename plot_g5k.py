...
sc2 = axes[1].scatter(tdp, gflops, c=price, cmap=cmap, norm=norm)


coeffs2 = np.polyfit(tdp, gflops, 2)
poly2 = np.poly1d(coeffs2)
axes[1].plot(x_range, poly2(x_range), color='red', linestyle='--', label="Quadratic regression")

texts = []
for i, name in enumerate(names):
    texts.append(axes[1].annotate(name, (tdp[i], gflops[i]), fontsize=12, ha='right',
                                  arrowprops=dict(arrowstyle="->", lw=0.5, color='gray')))
adjust_text(texts, only_move={'points':'xy', 'text':'xy'}, max_move=(10,10), ax=axes[1])
axes[1].set_xlabel('TDP')
axes[1].set_ylabel('GFLOPS')
axes[1].set_title('TDP vs GFLOPS')
axes[1].legend()
