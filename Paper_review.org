...
** Tuesday 11 february :
*** MLCA Tool :
I failed to use the tool via the github repository. But maybe it enough to compare the die size en memory size to observe that the edges device have the potential to have less impact compare to the large scale
**** Data :
...
*** Recall on the PBQ : Which conditions allow edge computing deployments to be more efficient than large-scale deployments for inference?
  + Efficiency : In our context we can consider efficiency with a QoE approach where the cost refer to the environemental impact + latencty.
**** Hypothesis :
+ by knowing that the die area of the Esge device that we want to study is less important than the ones used for large scale deployment we can expect that the manufacturing impact of edge devices have a les important impact.
...
+ "LLMs are notoriously difficult to quantize due to the outliers in the activations "
+ "The scale of outliers in activations is ∼ 100× larger than most of the activation values."
